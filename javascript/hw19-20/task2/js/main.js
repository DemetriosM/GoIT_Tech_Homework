...
// -----------------------data.json------------------------
 ...
 //    не должно быть повторяющихся скиллов, так же они должны быть отсортированы по алфавиту.
 
    var skills =_.union(_.flatten(_.map(data, 'skills')));
    console.log(_.sortBy(skills, function(value){
 ...
 // 2. Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (friends).
    ...
    var names = _.map(_.sortBy(data, function(value){
 ...
 // 3. Массив всех друзей всех пользователей, не должно быть повторяющихся людей.
    ...
    var friends =_.unionBy(_.flatten(_.map(data, 'friends')), 'name');
    console.log(_.map(friends, 'name'));
